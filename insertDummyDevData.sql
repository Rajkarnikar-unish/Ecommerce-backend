...
INSERT INTO address (address_line_1, city, country, local_user_id) VALUES ('123 Tester Hill', 'Testerton', 'England', @userId1);
INSERT INTO address (address_line_1, city, country, local_user_id) VALUES ('312 Spring Boot', 'Hibernate', 'England', @userId2);

DECLARE @address1 INT, @address2 INT;

SELECT TOP 1 @address1 = id FROM address WHERE local_user_id = @userId1 ORDER BY id DESC;
SELECT TOP 1 @address2 = id FROM address WHERE local_user_id = @userId2 ORDER BY id DESC;

INSERT INTO product_order (address_id, user_id) VALUES (@address1, @userId1);
INSERT INTO product_order (address_id, user_id) VALUES (@address1, @userId1);
INSERT INTO product_order (address_id, user_id) VALUES (@address1, @userId1);
INSERT INTO product_order (address_id, user_id) VALUES (@address2, @userId2);
INSERT INTO product_order (address_id, user_id) VALUES (@address2, @userId2);

DECLARE @order1 INT, @order2 INT, @order3 INT, @order4 INT, @order5 INT;

SELECT TOP 1 @order1 = id FROM product_order WHERE address_id = @address1 AND user_id = @userId1 ORDER BY id DESC
SELECT @order2 = id FROM product_order WHERE address_id = @address1 AND user_id = @userId1 ORDER BY id DESC OFFSET 1 ROW FETCH FIRST 1 ROW ONLY
SELECT  @order3 = id FROM product_order WHERE address_id = @address1 AND user_id = @userId1 ORDER BY id DESC OFFSET 2 ROW FETCH FIRST 1 ROW ONLY
SELECT TOP 1 @order4 = id FROM product_order WHERE address_id = @address2 AND user_id = @userId2 ORDER BY id DESC
SELECT @order5 = id FROM product_order WHERE address_id = @address2 AND user_id = @userId2 ORDER BY id DESC OFFSET 1 ROW FETCH FIRST 1 ROW ONLY
